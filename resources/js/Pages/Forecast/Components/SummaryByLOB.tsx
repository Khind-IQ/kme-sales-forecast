    ...
    <div className="bg-base-100 rounded-xl shadow-sm border border-base-300 overflow-hidden flex flex-col h-full max-h-[800px] animate-in fade-in duration-300">
        <div className="p-5 border-b border-base-300 bg-base-200/50 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-4">
            <h3 className="text-sm font-bold text-base-content uppercase tracking-wider">Summary By LOB</h3>
          ...
          <div className="flex items-center gap-3">
              <div className="text-xs text-base-content/60 font-bold bg-base-300 px-3 py-1.5 rounded-full">{lobSummaryData.filter(d => !d.isSubtotal).length} Rows</div>
              ...
              <button onClick={exportToCSV} disabled={lobSummaryData.length === 0} className="flex items-center gap-2 text-xs font-bold bg-base-100 border border-base-content/15 text-base-content/80 px-3 py-1.5 rounded-lg hover:bg-base-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm">
        ...
        <div className="overflow-auto flex-1 relative" aria-busy={isLoadingData}>
            ...
            <table className="w-full text-[12px] text-left border-collapse whitespace-nowrap">
                <thead className="sticky top-0 z-20 shadow-sm">
                    <tr>
                        ...
                        {visibleCols.itemCode && (
                            <th className="border border-base-content/15 bg-success-tint px-3 py-2 text-base-content font-bold hover:brightness-95 transition-colors cursor-pointer group" onClick={() => handleSort('itemCode')}>
                ...
                <tbody className="divide-y divide-base-200">
                    {paginatedData.map((item: any, idx: number) => {
                        ...
                        if (item.isSubtotal) {
                            return (
                                <tr key={item.unique_key} className="bg-base-300/50 transition-colors border-y border-base-content/15">
                                    ...
                                    {visibleCols.confirmed_qty && (
                                        <td className={`border border-base-content/15 px-3 py-2 text-center ${item.confirmed_qty > 0 ? 'bg-success/10 text-success' : 'text-base-content/30'}`}>{item.confirmed_qty > 0 ? item.confirmed_qty : '-'}</td>
                                    )}
                                    <td className={`border border-base-content/15 px-3 py-2 text-right ${item.forecast_qty > 0 ? 'bg-accent/5 text-base-content' : 'text-base-content/30'}`}>{item.forecast_qty > 0 ? item.net_sales.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) : '-'}</td>
                                </tr>
                            );
                        }
                        ...
                        return (
                            <tr key={item.unique_key} className="hover:bg-base-200 transition-colors">
                                ...
                                {/* HIERARCHY RENDERING */}
                                {visibleCols.lobName && isFirstLob && (<td rowSpan={lobRowSpan} className="border border-base-300 px-3 py-2 text-accent align-middle max-w-[150px] whitespace-normal leading-relaxed bg-base-100" title={item.lob_name}>{item.lob_name}</td>)}
                    ...
                    {lobSummaryData.length === 0 && <tr><td colSpan={activeColCount} className="p-10 text-center text-base-content/60 italic">{searchTerm ? 'No entries match your search.' : `No forecast entries found for ${monthFilter}.`}</td></tr>}
                </tbody>
                ...
                {lobSummaryData.length > 0 && (
                    <tfoot className="sticky bottom-0 z-20 shadow-[0_-1px_3px_rgba(0,0,0,0.05)] bg-base-150 font-bold text-base-content/80">
                        <tr>
                            ...
                            {visibleCols.confirmed_qty && (
                                <td className={`border border-base-content/15 px-3 py-2 text-center ${gridTotals.confirmed_qty > 0 ? 'bg-success/10 text-success' : 'text-base-content/60'}`}>{gridTotals.confirmed_qty > 0 ? gridTotals.confirmed_qty : '-'}</td>
                            )}
